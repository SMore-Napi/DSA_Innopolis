...
class Graph<V, E extends Comparable> {
    ...
    class Edge {
        ...
        public String toString() {
            return vertices.get(this.from).toString() + " -(" + this.weight.toString() + ")- " + vertices.get(this.to).toString();
        }
    }
    ...
    int addVertexNotExists(V vertex) {
    ...
    void addEdge(V from, V to, E weight) {
        int fromIndex = this.addVertexNotExists(from);
        int toIndex = this.addVertexNotExists(to);
        this.edges.add(new Edge(fromIndex, toIndex, weight));
    }
    ...
    public Graph<V, E> kruskalMST() {
        ...
        for (Edge edge : this.edges) {
            if (forest.find(edge.from) != forest.find(edge.to)) {
                ...
                mst.addEdge(this.vertices.get(edge.from), this.vertices.get(edge.to), edge.weight);
            }
        }
